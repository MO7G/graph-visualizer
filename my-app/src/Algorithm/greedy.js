class BinaryHeap {
  ...
  const greedyHelper = (gridData) => {
   ...
   let shortestPath =  greedyBestFirstSearch(grid,xStart,yStart,xEnd,yEnd,row,col,path);
  ...
  function greedyBestFirstSearch(grid, xStart, yStart, xEnd, yEnd, row, col, path) {
    ...
    queue.enqueue([xStart, yStart, calculateHeuristic(xStart, yStart, xEnd, yEnd)]);
    ...
    while (!queue.isEmpty()) {
      ...
      if (x === xEnd && y === yEnd) {
        ...
        const pathCoordinates = getPathCoordinates(parents, xStart, yStart, xEnd, yEnd);
  ...
  function getPathCoordinates(parents, xStart, yStart, xEnd, yEnd) {
    const pathCoordinates = [];
    let currentX = xEnd;
    let currentY = yEnd;
  
    while (currentX !== xStart || currentY !== yStart) {
      pathCoordinates.unshift([currentX, currentY]);
      const [parentX, parentY] = parents[currentX][currentY];
      currentX = parentX;
      currentY = parentY;
    }
  
    pathCoordinates.unshift([xStart, yStart]); // Add the starting node to the path
    return pathCoordinates;
  }
